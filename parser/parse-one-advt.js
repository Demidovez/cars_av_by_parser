...
// Возвращаем функцию парсинга одного объявления
module.exports = (htmlData, htmlUrl) => {
  ...
  // Поколение
  const generation = temp ? Helper.getGeneration(temp.innerText, series) : "";
  ...
  // Полная ссылка
  const full_url = new URL(htmlUrl);
  ...
  // Объем двигателя
  const volume_engine = temp ? Helper.getVolumeEngine(temp.innerText) : -1;
  ...
  // Фото
  temp = root.querySelectorAll(".gallery__stage-shaft .gallery__frame img");
  ...
  temp = root.querySelectorAll(".card__options-wrap .card__options-section");
